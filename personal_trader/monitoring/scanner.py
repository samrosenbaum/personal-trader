...
from personal_trader.analysis.macro import get_macro_snapshot, spx_correlation
...
class MarketScanner:
    ...
    def scan_once(self) -> ScanCycle:
        ...
        # Fetch macro snapshot once per cycle
        try:
            macro = get_macro_snapshot()
        except Exception as e:
            macro = None
            cycle.errors.append(f"Macro fetch failed: {e}")
    ...
    def _scan_symbol(
        ...
        correlation = spx_correlation(tf_data.get("1d")) if "1d" in tf_data else None
        ...
        futures_basis = None
        if base in ("BTC", "ETH"):
            vol_surface = self._get_cached_vol_surface(base)
            futures_basis = self._get_cached_futures_basis(base)
